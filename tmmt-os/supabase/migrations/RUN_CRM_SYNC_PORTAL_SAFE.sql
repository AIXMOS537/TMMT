-- CRM sync when full TMMT OS (0001_init) is not applied yet.
-- Safe to run on shared Supabase used by Rentals Portal.

create table if not exists public.cases (
  id uuid primary key default gen_random_uuid(),
  ref_code text unique default ('C-' || upper(substr(replace(gen_random_uuid()::text, '-', ''), 1, 8))),
