...
export default function PaymentsPage() {
  ...
  const load = () => { setLoading(true); setError(null); getPayments().then((d) => { setData(d as Pay[]); setLoading(false); }).catch(() => { setError("Failed to load data."); setLoading(false); }); };
  useEffect(load, []);
  ...
  return (
    <div>
      ...
      <Modal open={modalOpen} onClose={() => { setModalOpen(false); setEditing(null); setError(null); setSaving(false); }} title={editing ? "Edit Payment" : "Record Payment"}>
